...
namespace autocrypt_core::crypto {
    ...
    public:
        ...
        __forceinline ~StackDecrypter() noexcept {
            for (std::size_t i = 0; i <= CharCount; ++i) {
                reinterpret_cast<volatile CharT*>(storage.data())[i] = static_cast<CharT>(0);
            }
        }
...
#define AUTOSTR(str) autocrypt_core::crypto::StackDecrypter<autocrypt_core::crypto::FixedString(str), static_cast<std::uint32_t>(autocrypt_core::crypto::detail::Hash64(__TIMESTAMP__, sizeof(__TIMESTAMP__)) ^ __COUNTER__)>().Get()